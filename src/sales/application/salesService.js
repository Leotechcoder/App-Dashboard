...
export class SalesService {
  ...
  async getClosedOrders(startDate, endDate) {
    ...
    if (Array.isArray(res)) {
      ...
      totalEarnings = res.totalEarnings ?? SalesService.calculateTotalSales(orders)
  ...
  async openCashRegister(initialAmount, openedBy = "system") {
    const result = await this.repository.openCashRegister(initialAmount, openedBy)
    return new CashRegisterEntity(result).toObject()
  }

  async closeCashRegister(cashRegisterId, finalAmount, closedBy = "system", orders = []) {
    const closedRegister = await this.repository.closeCashRegister(cashRegisterId, finalAmount, closedBy)
  ...
  static calculateDiscrepancy(initialAmount = 0, finalAmount = 0, ordersTotal = 0) {
    const expectedAmount = Number(initialAmount || 0) + Number(ordersTotal || 0)
  ...
  static analyzeDiscrepancy(discrepancy) {
    ...
    if (Math.abs(discrepancy) <= eps) {
      return { type: "balanced", message: "La caja cerró correctamente", severity: "success" }
    } else if (discrepancy > 0) {
      return { type: "surplus", message: "La caja cerró con más dinero del esperado", severity: "warning" }
    } else {
      return { type: "deficit", message: "La caja cerró con menos dinero del esperado", severity: "error" }
    }
  }
  ...
  static filterOrdersByTimeRange(orders = [], startTime, endTime) {
    ...
    return orders.filter((order) => {
      const d = new Date(order.paidAt ?? order.paid_at ?? order.createdAt ?? order.created_at)
  ...
  static calculateSalesMetrics(orders = []) {
    ...
    const totalItems = orders.reduce((sum, order) => {
      if (typeof order.getTotalItems === "function") return sum + order.getTotalItems()
      if (Array.isArray(order.items)) return sum + order.items.reduce((s, it) => s + (it.quantity ?? it.qty ?? 0), 0)
      return sum
    }, 0)
  ...
  static calculateCashAmount(order) {
    ...
    const efectivo = amounts.efectivo ?? amounts.cash ?? amounts["efectivo"] ?? 0;
  ...
  static calculateTotalCashAmount(orders = []) {
    return orders.reduce((sum, order) => sum + this.calculateCashAmount(order), 0);
  }
